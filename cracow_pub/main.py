...
# function to show time interval between depart and current time
def time_interval(low, high):
    lower = datetime.strptime(low.strftime('%H%M'), '%H%M')
    higher = datetime.strptime(high.strftime('%H%M'), '%H%M')
    return higher - lower

# function that calls search again message after successful operation
def search_again():
    again = input("Press any key for search again or 'q' for quit: ")
    if again == 'q':
...
# function for managing user's choice of line number
def line_num():
    while True:
...
def dir_num():
    ...
    while True:
        drctn = input('Choose one: ').title()
        if drctn.isdigit():
            print("*******************************")
            print("Please enter direction's name. Not number!")
            print(f"It's '{list(week[ask_line])[0]}' or '{list(week[ask_line])[1]}'.")
            print("*******************************")
        elif drctn not in week[ask_line]:
            print("*******************************")
...
def station_select():
    ...
    while True:
        print("*******************************")
        stacja = input("So, where do we start? ==> ").title()
        if stacja.isdigit():
            print("*******************************")
            print("Enter station's name. Not digits!")
        elif stacja not in list(week[ask_line][ask_dir]):
            print("*******************************")
...
class Ttable:
    ...
    # function that determines day of the week, current time and accept user entries in order to process through
    # Lines.py and show
    # the closest trip for each line and how many time left until it departs
...
if __name__ == '__main__':
    ...
    while True:        
        try:
            ...
            # assign entries to class object Ttable for further maintaining
